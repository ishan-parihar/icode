# Container-first icode workflows
...
- `rust/crates/icode-cli/src/main.rs` exposes that state through the `icode sandbox` / `cargo run -p icode-cli -- sandbox` report.
...
```bash
docker build -t icode-dev -f Containerfile .
```

### Podman

```bash
podman build -t icode-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/icode-target \
  -w /workspace/rust \
  icode-dev \
...
cargo test --workspace
cargo run -p icode-cli -- --help
cargo run -p icode-cli -- sandbox
```
...
If you want to run `icode` against a second checkout while keeping `icode` itself mounted read-write:
...
```bash
cargo run -p icode-cli -- prompt "summarize /repo"
```
...
- Running with `CARGO_TARGET_DIR=/tmp/icode-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.
