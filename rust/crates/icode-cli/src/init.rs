...
const GITIGNORE_COMMENT: &str = "# icode local artifacts";
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to icode (clawcode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
