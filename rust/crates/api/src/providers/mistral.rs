...
impl MistralClient {
    ...
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty("MISTRAL_API_KEY")?
            .ok_or_else(|| ApiError::missing_credentials("Mistral", &["MISTRAL_API_KEY"]))?;
        Ok(Self::new(api_key))
    }
...
pub fn read_base_url() -> String {
    read_env_non_empty("MISTRAL_BASE_URL")
        .ok()
        .flatten()
...
pub fn has_api_key() -> bool {
    compat_has_api_key("MISTRAL_API_KEY")
}
...
fn openai_tool_choice(tool_choice: &ToolChoice) -> Value {
    match tool_choice {
        ...
        ToolChoice::Any => Value::String("any".to_string()),
...
mod tests {
    ...
    fn endpoint_url_construction() {
        let base = "https://api.mistral.ai/v1";
    ...
    fn has_api_key_detects_env() {
        ...
        assert!(!has_api_key());
        std::env::set_var("MISTRAL_API_KEY", "test-mistral-key");
        assert!(has_api_key());
        std::env::remove_var("MISTRAL_API_KEY");
    }

    #[test]
    fn resolves_mistral_model_alias_passthrough() {
        assert_eq!(
            resolve_model_alias("mistral/mistral-large-latest"),
