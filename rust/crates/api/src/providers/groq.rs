...
impl GroqClient {
    ...
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty("GROQ_API_KEY")?
            .ok_or_else(|| ApiError::missing_credentials("Groq", &["GROQ_API_KEY"]))?;
        Ok(Self::new(api_key))
    }
...
pub fn read_base_url() -> String {
    read_env_non_empty("GROQ_BASE_URL")
        .ok()
        .flatten()
...
pub fn has_api_key() -> bool {
    compat_has_api_key("GROQ_API_KEY")
}
...
mod tests {
    ...
    #[test]
    fn resolves_groq_provider_model_alias_passthrough() {
        assert_eq!(
